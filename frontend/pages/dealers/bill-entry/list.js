...
const BillEntryList = () => {
  ...
  const [dateFilterType, setDateFilterType] = useState('billDate'); // New state for filter type
  ...
  const handleSearch = (value) => {
    setSearchText(value);
    filterBills(value, selectedBranch, selectedDealer, selectedDateRange, dateFilterType);
  };

  const handleBranchFilter = (branchId) => {
    setSelectedBranch(branchId);
    filterBills(searchText, branchId, selectedDealer, selectedDateRange, dateFilterType);
  };

  const handleDealerFilter = (value) => {
    setSelectedDealer(value);
    filterBills(searchText, selectedBranch, value, selectedDateRange, dateFilterType);
  };

  const handleDateFilter = (dates) => {
    setSelectedDateRange(dates);
    filterBills(searchText, selectedBranch, selectedDealer, dates, dateFilterType);
  };

  const handleDateFilterTypeChange = (value) => {
    setDateFilterType(value);
    filterBills(searchText, selectedBranch, selectedDealer, selectedDateRange, value);
  };

  const filterBills = (search, branchId, dealerId, dateRange, filterType) => {
    let filtered = [...bills];
    console.log('Applying filter with:', { search, branchId, dealerId, dateRange, filterType });
